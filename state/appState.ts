...
import {
  ...
  ArmyValidationState,
  Detachment,
  DetachmentConfiguration,
  DetachmentName,
  DetachmentValidationState,
  Formation,
  FormationName,
  FormationShape,
  IconicDetachmentRequirementType,
  IconicFormationShape,
  ModelGroup,
  ModelLoadoutGroup,
  ModelName,
  Stats,
...
import {
  getDetachmentConfigurationForDetachmentName,
  getDetachmentNamesForSlot,
  getShapeForFormationName,
  getStatsForModelName,
} from "../game/lists.ts";
import {
  getArmyNamesLocally,
  loadArmyLocally,
...
export type ChangeFormationArmyList = (
  uuid: string,
  armyListName: ArmyListName | "",
) => void;
export type ChangeModelNumber = (
  uuid: string,
  detachmentIndex: number,
  modelName: ModelName,
  num: number,
) => void;
export type ChangeModelLoadout = (
  uuid: string,
  detachmentIndex: number,
  modelName: ModelName,
...
export type AddModelLoadoutGroup = (
  uuid: string,
  detachmentIndex: number,
  modelName: ModelName,
) => void;
export type RemoveModelLoadoutGroup = (
  uuid: string,
  detachmentIndex: number,
  modelName: ModelName,
  modelLoadoutGroupIndex: number,
) => void;
export type ChangeModelLoadoutGroupNumber = (
  uuid: string,
  detachmentIndex: number,
  modelName: ModelName,
  modelLoadoutGroupIndex: number,
...
export type AppStateType = {
  ...
  changeIconicFormationRequirement: (
    uuid: string,
  ...
  changeDetachmentName: (
    uuid: string,
    detachmentIndex: number,
    detachmentName: DetachmentName | "",
  ) => void;
  changeDetachmentAttachment: (
    uuid: string,
    detachmentIndex: number,
...
function calcModelLoadoutGroupPoints(modelLoadoutGroup: ModelLoadoutGroup) {
  ...
  return modelLoadoutGroup.number *
    modelLoadoutGroup.modelLoadoutSlots.reduce(
      (p2, m2) => p2 + m2.modelLoadout.points,
      0,
    );
}

function calcModelGroupPoints(
  armyListName: ArmyListName,
  modelGroup: ModelGroup,
  detachmentName: DetachmentName,
) {
  //add up the points for the models and the loadouts. Model points require looking up.
  const config = getDetachmentConfigurationForDetachmentName(
    armyListName,
    detachmentName,
  );
  const c = config?.modelGroupShapes.find((x) =>
    x.modelName == modelGroup.modelName
  );
  const points =
    c?.possibleModelGroupQuantities.find((m) => m.num == modelGroup.number)
      ?.points ?? 0;
  return modelGroup.modelLoadoutGroups.reduce((p, m) => p + m.points, 0) +
    points;
}

function calcDetachmentPoints(detachment: Detachment) {
  return detachment.modelGroups.reduce((p, m) => p + m.points, 0) +
    (detachment.extras?.reduce((p, e) => p + (e.has ? e.points : 0), 0) ?? 0);
}
...
function calcFormationBreakPoint(formation: Formation): number {
  const wounds = formation.detachments.reduce((p, d) => {
    ...
    return p + d.modelGroups.reduce((p2, mg) => {
      ...
      if (stats !== undefined) {
        if (
          stats.detachmentType == "Knight" || stats.detachmentType == "Titan"
        ) {
        ...
        if (
          stats.unitTraits.find((x) =>
            x == "Automated Sentry" || x == "Drop Pod"
          ) != undefined
        ) {
          woundsPerModel = 0;
        }
...
function calcFormationActivations(formation: Formation) {
  return formation.detachments.reduce((a, d) => {
    ...
    const isAttachedDetachment = d.attachedDetachmentIndex != undefined &&
      d.attachedDetachmentIndex != -1;
    ...
    if (formation.armyListName != "" && d.detachmentName != "") {
      const c = getDetachmentConfigurationForDetachmentName(
        formation.armyListName,
        d.detachmentName,
      );
      for (const mg of d.modelGroups) {
        if (mg.number > 0) {
          const mgs = c.modelGroupShapes.find((t) =>
            t.modelName == mg.modelName
          );
          if (mgs != undefined) {
            ...
            if (mgs.dedicatedTransport) {
              ...
              const stats = getStatsForModelName(mg.modelName);
              if (
                (mgs.unitTraits != undefined && mgs.unitTraits.findIndex((t) =>
                      t == "Independent"
                    ) != -1) ||
            ...
            console.error(
              "Could not find model group shape for " + mg.modelName +
                " in detachment " + d.detachmentName + " of formation " +
    ...
    if (hasIndependentModels) {
      ...
      return a + independentModels + independentModelGroups +
        (hasDedicatedTransports ? 1 : 0);
    } else {
      ...
      return a + (hasNormalModelGroup ? 1 : 0) + independentModelGroups +
        (hasDedicatedTransports ? 1 : 0);
    }
  }, 0);
}
...
function calculateTransportData(
  stats: Stats,
  modelGroup: ModelGroup,
  ...
  for (const mlg of modelGroup.modelLoadoutGroups) {
    ...
    for (const slot of mlg.modelLoadoutSlots) {
      const statsSlot = stats.modelLoadoutSlots.find((s) =>
        s.name == slot.name
      );
      const statsLoadout = statsSlot?.possibleModelLoadouts.find((s) =>
        s.loadout == slot.modelLoadout.loadout
      );
    ...
    if (transportTrait) {
        ...
        transports.push(
          {
            ...
            remainingCapacity: transportCapacity,
            takesWalkers: false,
            takesBulky: false,
            bulkyIs: 0,
          },
      ...
      const assaultTransportTrait = traits.find((x) =>
        x.startsWith("Assault Transport")
      );
      if (assaultTransportTrait) {
        const matches = /Assault Transport \(([0-9])\)/.exec(
          assaultTransportTrait,
        );
        const transportCapacity = parseInt(matches![1]);
        for (let i = 0; i < numberModels; ++i) {
          transports.push(
        ...
        const largeTransportTrait = traits.find((x) =>
          x.startsWith("Large Transport") ||
          x.startsWith("Large Assault Transport")
        );
...
function calcDetachmentCommanderValidation(
  formation: Formation,
  detachmentIndex: number,
    ...
    if (detachment.attachedDetachmentIndex != undefined) {
      ...
      for (let i = 0; i < formation.detachments.length; ++i) {
        ...
        if (otherDetachment.modelGroups.length > 0) {
          const otherStats = getStatsForModelName(
            otherDetachment.modelGroups[0].modelName,
          );
          if (otherStats != undefined) {
            if (statsHasTrait(otherStats, "Commander")) {
              return {
                valid: false,
                error: "Multiple Commanders in Formation",
              };
            }
...
function calcDetachmentAttachedDeploymentValidation(
  formation: Formation,
  detachmentIndex: number,
  detachment: Detachment,
  stats: Stats | undefined,
    ...
    if (detachment.attachedDetachmentIndex != undefined) {
      ...
      for (let i = 0; i < formation.detachments.length; ++i) {
        ...
        if (otherDetachment.modelGroups.length > 0) {
          ...
          if (otherStats != undefined) {
            if (
              detachment.attachedDetachmentIndex == -1 &&
              otherStats?.detachmentType == stats.detachmentType
            ) {
              return {
                valid: false,
                error: "Attached Deployment not attached to detachment",
              };
            }
          }
        }
...
type ModelNumbersForDetachment = {
  numModels: number;
  walkerModels: number;
  veryBulkyModels: number;
  bulkyModels: number;
  slimModels: number;
};

function recordModelNumbersForModelGroup(
  out: ModelNumbersForDetachment,
  modelGroup: ModelGroup,
  stats: Stats | undefined,
): void {
  const unitTraits = stats?.unitTraits ?? [];
  ...
  if (stats) {
    if (stats.detachmentType == "Walker") {
      ...
      const isBulky = unitTraits.findIndex((x) =>
        x === "Bulky" || x === "Jump Packs"
      ) !== -1;
      if (isBulky) {
        out.bulkyModels += modelGroup.number;
      } else {
        const isVeryBulky = unitTraits.findIndex((x) =>
          x === "Very Bulky"
        ) !== -1;
        if (isVeryBulky) {
          out.veryBulkyModels += modelGroup.number;
        } else {
          out.slimModels += modelGroup.number;
        }
      }
    } else if (
      stats.detachmentType == "Vehicle" && statsHasTrait(stats, "Compact")
    ) {
      out.bulkyModels += modelGroup.number;
    }
  }
...
function calcDetachmentValidation(
  formation: Formation,
  detachmentIndex: number,
  ...
  if (
    slotRequirements.slotRequirementType == "Required" &&
    detachment.modelGroups.length === 0
  ) {
  ...
  if (formation.armyListName != "") {
    if (detachment.detachmentName != "") {
      ...
      for (let i = 0; i < detachment.modelGroups.length; ++i) {
        ...
        const shapeIdx = c.modelGroupShapes.findIndex((x) =>
          x.modelName == modelGroup.modelName
        );
        if (shapeIdx == -1) {
          continue;
        ...
        if (
          shape.minModels != undefined && modelGroup.number < shape.minModels
        ) {
          return {
            valid: false,
            error: "Too few models in group",
            data: modelGroup.modelName + ". Min: " + shape.minModels,
          };
        }
        if (
          shape.maxModels != undefined && modelGroup.number > shape.maxModels
        ) {
          return {
            valid: false,
            error: "Too many models in group",
            data: modelGroup.modelName + ". Max: " + shape.maxModels,
          };
        }
        if (
          shape.possibleModelGroupQuantities.findIndex((x) =>
            x.num == modelGroup.number
          ) == -1
        ) {
          const possibleModelNumbers = shape.possibleModelGroupQuantities.map(
            (m) => m.num.toString()
          );
          return {
            valid: false,
            error: "Invalid number of models in group",
            data: modelGroup.modelName + ". Could be: " +
        ...
        const commanderValidation = calcDetachmentCommanderValidation(
          formation,
          detachmentIndex,
          detachment,
          stats,
        ...
        const attachedDeploymentValidation =
          calcDetachmentAttachedDeploymentValidation(
            formation,
            detachmentIndex,
            detachment,
            stats,
        ...
        if (
          shape.dedicatedTransport === undefined ||
          shape.dedicatedTransport === false
        ) {
          //not a transport, so could go in a transport
          recordModelNumbersForModelGroup(
            modelNumbersForDetachment,
            modelGroup,
            stats,
          );
      ...
      if (
        c.maxModels !== undefined &&
        c.maxModels < modelNumbersForDetachment.numModels
      ) {
        return {
          valid: false,
          error: "Too many models in detachment",
          data: "max " + c.maxModels,
        };
      }
      if (
        c.minModels !== undefined &&
        c.minModels > modelNumbersForDetachment.numModels
      ) {
        return {
          valid: false,
          error: "Too few models in detachment",
          data: "min " + c.minModels,
        };
      }

      //okay now we need to get any attached detachment numbers
      const attachedDetachment = formation.detachments.find((f) =>
        f.attachedDetachmentIndex == detachmentIndex
      );
      if (attachedDetachment != null) {
        for (let i = 0; i < attachedDetachment.modelGroups.length; ++i) {
          ...
          const attachedConfiguration =
            getDetachmentConfigurationForDetachmentName(
              formation.armyListName,
              attachedDetachment.detachmentName,
            );
          const attachedShape = attachedConfiguration.modelGroupShapes.find(
            (x) => x.modelName == attachedModelGroup.modelName
          );
          if (attachedShape == undefined) {
            continue;
          }
          const attachedStats = getStatsForModelName(
            attachedModelGroup.modelName,
          );
          //scoop up numbers of models that are attached
          if (
            attachedShape.dedicatedTransport === undefined ||
            attachedShape.dedicatedTransport === false
          ) {
            recordModelNumbersForModelGroup(
              modelNumbersForDetachment,
              attachedModelGroup,
      ...
      if (
        transports.length > 0 &&
        (detachment.attachedDetachmentIndex == undefined ||
          detachment.attachedDetachmentIndex == -1)
      ) {
        //work down from walkers to slims
        while (modelNumbersForDetachment.walkerModels > 0) {
          const transport = transports.find((x) =>
            x.takesWalkers && x.remainingCapacity >= 2
          );
          if (transport === undefined) {
            return {
              valid: false,
              error: "Need more dedicated transports",
              data: "remaining walkers: " +
                modelNumbersForDetachment.walkerModels,
            };
          }
        ...
        //now very bulky
        while (modelNumbersForDetachment.veryBulkyModels > 0) {
          //Use x.bulkyIs == 1 as a proxy for Large Transport or Large Assault Transport
          const transport = transports.find((x) =>
            x.takesBulky && x.bulkyIs == 1 && x.remainingCapacity >= 2
          );
          if (transport === undefined) {
            return {
              valid: false,
              error: "Need more dedicated transports",
              data: "remaining very bulky models: " +
                modelNumbersForDetachment.veryBulkyModels,
            };
          }

          const bulkyLoad = modelNumbersForDetachment.veryBulkyModels * 2;
          ...
          if (transport.remainingCapacity % 2 == 0) {
            ...
            const numToLoad = Math.floor(transport.remainingCapacity / 2);
            transport.remainingCapacity -= numToLoad * 2;
            modelNumbersForDetachment.veryBulkyModels -= numToLoad;
          }
        }
        //now bulky
        while (modelNumbersForDetachment.bulkyModels > 0) {
          const transport = transports.find((x) =>
            x.takesBulky && x.remainingCapacity >= x.bulkyIs
          );
          if (transport === undefined) {
            return {
              valid: false,
              error: "Need more dedicated transports",
              data: "remaining bulky models: " +
                modelNumbersForDetachment.bulkyModels,
            };
          }

          const bulkyLoad = modelNumbersForDetachment.bulkyModels *
            transport.bulkyIs;
          if (transport.remainingCapacity >= bulkyLoad) {
            transport.remainingCapacity -= bulkyLoad;
            break;
          ...
          if (transport.remainingCapacity % transport.bulkyIs == 0) {
            modelNumbersForDetachment.bulkyModels -=
              transport.remainingCapacity / transport.bulkyIs;
            transport.remainingCapacity = 0;
          } else {
            const numToLoad = Math.floor(
              transport.remainingCapacity / transport.bulkyIs,
            );
        ...
        while (modelNumbersForDetachment.slimModels > 0) {
          ...
          if (transport === undefined) {
            return {
              valid: false,
              error: "Need more dedicated transports",
              data: "remaining models: " + modelNumbersForDetachment.slimModels,
            };
          }
          if (
        ...
        const transport = transports.find((x) =>
          x.remainingCapacity == x.capacity
        );
        if (transport !== undefined) {
      ...
      if (
        slotRequirements.slotRequirementType == "One Of" &&
        ...
        for (let i = 0; i < formation.detachments.length; i++) {
          ...
          if (
            formation.detachments[i].detachmentName != "" &&
            formationShape.slotRequirements[i].slotRequirementType ==
              "One Of" &&
            formationShape.slotRequirements[i].oneOfGroup == g
          ) {
            return {
              valid: false,
              error: "Only one of these detachments should be present",
            };
          }
        }
      } else if (
        (slotRequirements.slotRequirementType == "One Of Group" ||
          slotRequirements.slotRequirementType == "Required One Of Group") &&
        slotRequirements.oneOfGroup !== undefined &&
  ...
  if (formationShape.customValidation != undefined) {
    const validationState = formationShape.customValidation(
      formation,
      detachmentIndex,
    );
    if (validationState.error) {
...
function refreshFormation(newFormation: Formation) {
  const formationShape = getShapeForFormationName(
    newFormation.armyListName,
  ...
  if (formationShape.formationType === "Iconic") {
    ...
    if (
      newFormation.legionName !== undefined &&
      formationShape.legionName !== undefined &&
      newFormation.legionName !== formationShape.legionName
    ) {
      newFormation.detachments[0].validationState = {
        valid: false,
        error: "Invalid Legion name",
        data: "Must be " + formationShape.legionName,
      };
    }
  } else {
    newFormation.detachments.forEach(
      (x, i) => {
        ...
        x.validationState = calcDetachmentValidation(
          newFormation,
          i,
          formationShape,
        );
...
function calcArmyValidation(army: Army): ArmyValidationState {
  ...
  for (const formation of army.formations) {
    ...
    if (
      formationShape.formationType == "Normal" ||
      formationShape.formationType == "Iconic"
    ) {
      numNormalFormations++;
...
function getDefaultModelGroupsForDetachment(
  config: DetachmentConfiguration,
  formationName: FormationName,
): ModelGroup[] {
  return config.modelGroupShapes
    .filter((x) =>
      (x.formationNames === undefined) ||
      (x.formationNames.findIndex((s) => s == formationName) != -1)
    )
    .map((x) => {
      let modelLoadoutGroups: ModelLoadoutGroup[] = [];
      if (x.modelLoadoutSlots.length > 0) {
        modelLoadoutGroups = [{
          number: x.possibleModelGroupQuantities[0].num,
          modelLoadoutSlots: x.modelLoadoutSlots.map(
            (y) => {
...
function defaultArmyName(): string {
  ...
  switch (r1) {
    case 0:
      return "The " + takeRandom(pluaralNouns) + " of " +
        takeRandom(singularProperNouns);
    case 1:
      return "The " + takeRandom(adjectives) + " " + takeRandom(pluaralNouns) +
        " of " + takeRandom(singularProperNouns);
    case 2:
      return "The " + takeRandom(adjectives) + " " + takeRandom(pluaralNouns);
    case 3:
      return "The " + takeRandom(singularNouns) + " " +
        takeRandom(pluaralNouns);
    case 4:
      return "The " + takeRandom(adjectives) + " " + takeRandom(singularNouns) +
        " " + takeRandom(pluaralNouns);
    default:
      return "Are We The Baddies?";
  }
}

function generateIconicFormationDetachments(
  formationShape: IconicFormationShape,
  newFormation: Formation,
): Detachment[] {
  return formationShape.iconicDetachments
    .filter((s) =>
      s.iconicDetachmentRequirementType == "Standard" ||
      newFormation.iconicDetachmentRequirementType == "Expanded"
    )
    .map((s) => {
      const out: Detachment = {
        slot: s.slot,
        modelGroups: s.modelGroups.map((t) => {
          const out: ModelGroup = {
            modelName: t.modelName,
            number: 0,
            points: 0,
            unitTraits: [],
            modelLoadoutGroups: t.modelLoadoutGroups.map((u) => {
              return {
                points: 0,
                number: u.number,
                modelLoadoutSlots: u.modelLoadoutSlots.map((v) => {
                  return {
                    name: v.name,
                    modelLoadout: { loadout: v.loadout, points: 0 },
                  };
                }),
              };
            }),
          };
          out.number = out.modelLoadoutGroups.reduce<number>(
        ...
        points: 0,
        detachmentName: s.detachmentName,
...
function createAppState(): AppStateType {
  const newArmy = {
    ...
    maxPoints: 0,
    primaryArmyListName: "" as ArmyListName | "",
  ...
  const save = async () => {
    if (armyHasName()) {
      if (armyLoadSource.value == ArmyLoadSource.Local) {
        ...
        armyLoadSource.value == ArmyLoadSource.KV && isLoggedIn.value &&
        username.value == kvArmyOwner.value
      ) {
  ...
  const canMoveToKvFn = () => {
    return isLoggedIn.value && armyLoadSource.value == ArmyLoadSource.Local &&
      armyHasName();
  };
  ...
  const changeArmyMaxPoints = (value: number) => {
    ...
    const points = calcArmyPoints(
      newArmy.formations,
      newArmy.primaryArmyListName,
    );
  ...
  const removeFormation: RemoveFormation = (uuid: string) => {
    ...
    newArmy.formations = newArmy.formations.filter((elem: Formation) =>
      elem.uuid != uuid
    );
    const points = calcArmyPoints(
      newArmy.formations,
  ...
  const changeFormationArmyList: ChangeFormationArmyList = (
    uuid: string,
    armyListName: ArmyListName | "",
  ) => {
    const formationIdx = army.value.formations.findIndex((f: Formation) =>
      f.uuid == uuid
    );
    if (formationIdx === -1) {
      return;
  ...
  const changeIconicFormationRequirement = (
    uuid: string,
    iconicDetachmentRequirementType: IconicDetachmentRequirementType,
  ) => {
    const formationIdx = army.value.formations.findIndex((f: Formation) =>
    ...
    if (
      army.value.formations[formationIdx].iconicDetachmentRequirementType ==
        iconicDetachmentRequirementType
    ) {
      return;
    }
    ...
    newFormation.iconicDetachmentRequirementType =
      iconicDetachmentRequirementType;
    ...
    newFormation.detachments = generateIconicFormationDetachments(
      formationShape,
      newFormation,
    );
  ...
  const changeFormationName = (
    ...
    if (formationShape.formationType == "Iconic") {
      ...
      newFormation.detachments = formationShape.slotRequirements.map((s) => {
        ...
        if (
          s.slotRequirementType == "Required" && newFormation.armyListName != ""
        ) {
          const dtfs = getDetachmentNamesForSlot(
            newFormation.armyListName,
            s.slot,
            army.value.allegiance,
          );
          if (dtfs.length == 1 && newFormation.formationName != "") {
            ...
            const out: Detachment = {
              slot: s.slot,
              modelGroups: getDefaultModelGroupsForDetachment(
                config,
                newFormation.formationName,
              ),
              points: 0,
              detachmentName: dtfs[0],
              validationState: { valid: true },
            };
            if (out.modelGroups.length > 0) {
              ...
              if (
                stats &&
                (statsHasTrait(stats, "Commander") ||
                  statsHasTrait(stats, "Attached Deployment"))
              ) {
                out.attachedDetachmentIndex = -1;
              }
            }
            if (config.extras != undefined) {
              out.extras = config.extras.map((e) => {
                return { name: e.name, points: e.points, has: false };
              });
            }
            return out;
          }
  ...
  const changeDetachmentName = (
    ...
    if (
      army.value.formations[formationIdx].detachments[detachmentIndex]
        .detachmentName == detachmentName
    ) {
      return;
    }
    ...
    if (newFormation.armyListName != "") {
      if (detachmentName != "") {
        ...
        if (newFormation.formationName != "" && config?.modelGroupShapes) {
          newFormation.detachments[detachmentIndex].modelGroups =
            getDefaultModelGroupsForDetachment(
              config,
              newFormation.formationName,
            );

          if (
            newFormation.detachments[detachmentIndex].modelGroups.length > 0
          ) {
            const stats = getStatsForModelName(
              newFormation.detachments[detachmentIndex].modelGroups[0]
                .modelName,
            );
            if (
              stats &&
              ...
              newFormation.detachments[detachmentIndex]
                .attachedDetachmentIndex = -1;
            }
          }

          if (config.extras != undefined) {
            newFormation.detachments[detachmentIndex].extras = config.extras
              .map((e) => {
                return { name: e.name, points: e.points, has: false };
              });
          } else {
        ...
        const shape = getShapeForFormationName(
          newFormation.armyListName,
          newFormation.formationName,
        );
  ...
  const changeDetachmentAttachment = (
    uuid: string,
    detachmentIndex: number,
    attachedDetachmentIndex: number,
  ) => {
    ...
    if (
      army.value.formations[formationIdx].detachments[detachmentIndex]
        .attachedDetachmentIndex == attachedDetachmentIndex
    ) {
      return;
    }

    const newFormation = structuredClone(army.value.formations[formationIdx]);
    newFormation.detachments[detachmentIndex].attachedDetachmentIndex =
      attachedDetachmentIndex;

    setFormationAtIdx(newFormation, formationIdx);
  };

  const changeExtraHas = (
    uuid: string,
    detachmentIndex: number,
    extraName: string,
    has: boolean,
    ...
    const extraIdx = army.value.formations[formationIdx]
      .detachments[detachmentIndex].extras.findIndex((e) =>
        e.name == extraName
      );
    if (extraIdx === -1) {
      return;
  ...
  const changeModelNumber: ChangeModelNumber = (
    uuid: string,
    detachmentIndex: number,
    modelName: ModelName,
    num: number,
    ...
    const modelGroupIdx = army.value.formations[formationIdx]
      .detachments[detachmentIndex].modelGroups.findIndex((m: ModelGroup) =>
    ...
    const newFormation = structuredClone(army.value.formations[formationIdx]);
    newFormation.detachments[detachmentIndex].modelGroups[modelGroupIdx]
      .number = num;

    const detachmentName =
      newFormation.detachments[detachmentIndex].detachmentName;
    if (detachmentName === "") {
      newFormation.detachments[detachmentIndex].modelGroups[modelGroupIdx]
        .points = 0;
      newFormation.detachments[detachmentIndex].modelGroups[modelGroupIdx]
      ...
      if (
        newFormation.detachments[detachmentIndex].modelGroups[modelGroupIdx]
          .modelLoadoutGroups.length === 1
      ) {
        newFormation.detachments[detachmentIndex].modelGroups[modelGroupIdx]
          .modelLoadoutGroups[0].number = num;
      }
    }
  ...
  const changeModelLoadout: ChangeModelLoadout = (
    ...
    const modelLoadoutSlotIndex = c?.modelLoadoutSlots.findIndex((s) =>
      s.name === modelLoadoutSlotName
    );
    if (modelLoadoutSlotIndex === undefined || modelLoadoutSlotIndex === -1) {
      return;
    }
    const l = c?.modelLoadoutSlots[modelLoadoutSlotIndex].possibleModelLoadouts
      .find((m) => m.loadout === loadout);
    if (l === undefined) {
      return;
    }

    newFormation.detachments[detachmentIndex].modelGroups[modelGroupIdx]
      .modelLoadoutGroups[modelLoadoutGroupIndex]
      .modelLoadoutSlots[modelLoadoutSlotIndex].modelLoadout = l;

    setFormationAtIdx(newFormation, formationIdx);
  };

  const addModelLoadoutGroup: AddModelLoadoutGroup = (
    uuid: string,
    detachmentIndex: number,
    modelName: ModelName,
  ) => {
    ...
    const modelGroupIndex = army.value.formations[formationIdx]
      .detachments[detachmentIndex].modelGroups.findIndex((m: ModelGroup) =>
        m.modelName == modelName
      );
    ...
    const newModelLoadoutGroup: ModelLoadoutGroup = {
      number: 0,
      modelLoadoutSlots: c.modelLoadoutSlots.map((y) => {
        return { name: y.name, modelLoadout: y.possibleModelLoadouts[0] };
      }),
      points: -1,
    };
    ...
    newFormation.detachments[detachmentIndex].modelGroups[modelGroupIndex]
      .modelLoadoutGroups.push(newModelLoadoutGroup);

    setFormationAtIdx(newFormation, formationIdx);
  };

  const removeModelLoadoutGroup: RemoveModelLoadoutGroup = (
    uuid: string,
    detachmentIndex: number,
    modelName: ModelName,
    modelLoadoutGroupIndex: number,
    ...
    newFormation.detachments[detachmentIndex].modelGroups[modelGroupIndex]
      .modelLoadoutGroups = newFormation.detachments[detachmentIndex]
        .modelGroups[modelGroupIndex].modelLoadoutGroups.filter((_, i) =>
          i != modelLoadoutGroupIndex
        );

    setFormationAtIdx(newFormation, formationIdx);
  };

  const changeModelLoadoutGroupNumber: ChangeModelLoadoutGroupNumber = (
    uuid: string,
    detachmentIndex: number,
    modelName: ModelName,
    modelLoadoutGroupIndex: number,
    ...
    newFormation.detachments[detachmentIndex].modelGroups[modelGroupIndex]
      .modelLoadoutGroups[modelLoadoutGroupIndex].number = number;
  ...
  return {
    ...
    changeArmyName,
    changeArmyMaxPoints,
    changePrimaryArmyListName,
    changeArmyAllegiance,
    addFormation,
    removeFormation,
    changeFormationArmyList,
    changeFormationLegionName,
    changeIconicFormationRequirement,
    changeFormationName,
    changeDetachmentName,
    changeDetachmentAttachment,
    ...
    changeExtraHas,
    changeModelLoadout,
    addModelLoadoutGroup,
    removeModelLoadoutGroup,
    changeModelLoadoutGroupNumber,
    canUndo,
    undo,
    canRedo,
    redo,
